...
export function auth(
  ...
  if (employeeAccessEnabled) {
    if (apiKey) {
      return {
        error: true,
        msg: "employee access key is required",
      };
    }
    ...
    if (!employeeAccess.ok || !employeeAccess.employee) {
      return {
        error: true,
        msg: employeeAccess.reason ?? "wrong employee access key",
      };
    }
  ...
  if (!apiKey) {
    ...
    console.log("[Auth] use user api key", maskSecret(apiKey));
  }
