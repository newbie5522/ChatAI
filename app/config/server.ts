...
declare global {
  namespace NodeJS {
    interface ProcessEnv {
      ...
      ALLOW_USER_PROVIDER_KEYS?: string; // allow user's api key input in company mode
      DISABLE_GPT4?: string; // allow user to use gpt-4 or not
      ENABLE_BALANCE_QUERY?: string; // allow user to query balance or not
      DISABLE_FAST_LINK?: string; // disallow parse settings from url or not
      ALLOW_FAST_LINK_SETTINGS?: string; // allow settings/code import from url in company mode
...
export const getServerSideConfig = () => {
  ...
  return {
    ...
    needCode: employeeAccessEnabled || ACCESS_CODES.size > 0,
    code: process.env.CODE,
    codes: ACCESS_CODES,
    employeeAccessEnabled,
    employeeAccessCount: getEmployeeAccessCount(),

    proxyUrl: process.env.PROXY_URL,
    isVercel: !!process.env.VERCEL,

    hideUserApiKey: process.env.ALLOW_USER_PROVIDER_KEYS !== "1",
    disableGPT4,
    hideBalanceQuery: !process.env.ENABLE_BALANCE_QUERY,
    disableFastLink: process.env.ALLOW_FAST_LINK_SETTINGS !== "1",
    customModels,
    defaultModel,
    visionModels,
