...
export function RealtimeConfigList(props: {
  ...
  const { hideApiKey, realtimeConfig, updateConfig } = props;

  useEffect(() => {
    if (hideApiKey && realtimeConfig.apiKey) {
      updateConfig((config) => (config.apiKey = ""));
    }
  }, [hideApiKey, realtimeConfig.apiKey, updateConfig]);
